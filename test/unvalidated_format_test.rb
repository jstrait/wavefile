...
class UnvalidatedFormatTest < Minitest::Test
  ...
  def test_speaker_mapping_more_speakers_defined_than_channels
    ...
    # Even though more than 3 speakers are set in the bit field, only the first 3 should be included in the mapping
    assert_equal([:front_right, :front_center, :back_left], unvalided_format.speaker_mapping)
  end
