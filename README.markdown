...
* **Can read `smpl` chunk data from files that contain this kind of chunk.** If present, this will be available via `#sampler_info` on a constructed `Reader` instance (otherwise it will be `nil`). Thanks to [@henrikj242](https://github.com/henrikj242) for suggesting this feature and providing the base implementation.
